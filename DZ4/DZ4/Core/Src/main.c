...
volatile uint8_t cmd_recieved = 0;
uint8_t txData [3] = {0};
uint8_t rxData [3]= {0};
/* USER CODE END PV */
...
uint16_t crc_mb(uint8_t *buf, int len)
{
    ...
    // Low and high bytes may need to be swapped depending on usage
    return crc;
}
...
int main(void)
{
  ...
  while (1)
  {
	  if (btn_pressed)
	  {
		  HAL_UART_Transmit(&huart1, txData, 3, HAL_MAX_DELAY);
		  btn_pressed = 0;
	  }

	  if (cmd_recieved)
	  {
		  uint16_t crc = crc_mb(rxData, 1);

	      if ((rxData[1] == (uint8_t)(crc >> 8)) && (rxData[2] == (uint8_t)(crc & 0xFF)))
	      {
	    	  if (rxData[0] == 0x66)
	    	  {
